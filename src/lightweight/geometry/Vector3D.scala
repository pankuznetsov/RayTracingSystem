...
case class Vector3D(x: Float, y: Float, z: Float) extends RootType() {

  @inline def +(other: Vector3D) = Vector3D(x + other.x, y + other.y, z + other.z)
  @inline def -(other: Vector3D) = Vector3D(x - other.x, y - other.y, z - other.z)
  @inline def *(other: Vector3D) = Vector3D(x * other.x, y * other.y, z * other.z)
  @inline def *(scalar: Float) = Vector3D(x * scalar, y * scalar, z * scalar)
  @inline def /(other: Vector3D) = Vector3D(x / other.x, y / other.y, z / other.z)
  @inline def /(scalar: Float) = Vector3D(x / scalar, y / scalar, z / scalar)

  @inline def dotProduct(other: Vector3D): Float = x * other.x + y * other.y + z * other.z

  @inline def crossProduct(other: Vector3D) = Vector3D(y * other.z - z * other.y, z * other.x - x * other.z, x * other.y - y * other.x)

  @inline def magnitude: Float = sqrt(pow(x, 2) + pow(y, 2) + pow(z, 2)).asInstanceOf[Float]
  ...
  @inline def refract(normal: Vector3D, inIOR: Float, outIOR: Float): Vector3D = {
    this + normal * this.dotProduct(normal) * (sqrt((outIOR - inIOR) / this.dotProduct(normal) + 1) - 1).asInstanceOf[Float]
  }
  ...
  @inline def linearInterpolation(other: Vector3D, mix: Float): Vector3D = (this * (1 - mix)) + (other * mix)
