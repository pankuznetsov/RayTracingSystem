...
case class PointLight(override val location: Vector3D, override val size: Double, override val maxDistance: Double, override val output: LampOutput, override val directlyVisible: Boolean) extends Lamp(location, size, maxDistance, output, directlyVisible) {
