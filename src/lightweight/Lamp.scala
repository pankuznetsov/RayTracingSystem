...
abstract class Lamp(val location: Vector3D, val size: Double, val maxDistance: Double, val output: LampOutput, val directlyVisible: Boolean, val samples: Int) {
   ...
   */
  def isCollide(mesh: Mesh, triangleIndex: Int, ray: Ray): (Vector3D, Double, Double)
