...
case class RayTeleport(override val inputs: Array[Container], override val outputs: Array[Container]) extends Node(inputs, outputs) {
  ...
  override def doThings(mesh: Mesh, world: World, triangleIndex: Int, ray: Ray, hitPoint: Vector3D, coordinates: Vector3D, backColor: Color, shadersLeft: Int, rayOriginInfo: RayOriginInfo): Unit = {
    if (mesh.binder.contains(triangleIndex)) {
      ...
      if (bind.nonEmpty) {
        val barycentric: (Float, Float, Float) = mesh.mesh(triangleIndex).getBarycentric(hitPoint)
        ...
        val rayColor = teleportedRay.renderSample(mesh, world, bind.get, shadersLeft, RayOriginInfo(this, false))
